...
Reemplaza a las guías anteriores: **el encabezado del Recorder tiene que decir v0.9.10**.
...
## Parte A — Lo que cambió en esta versión
...
> ⚠️ **Ojo, esto cambió respecto de la guía anterior.** La 0.9.9 decía que apenas una persona
> identificaba un título quedaba identificado para todos. **Ya no es así**, y es a propósito:
> ver A.6. Ahora hace falta que **dos personas distintas** declaren lo mismo. Mientras tanto,
> quien lo declaró lo sigue viendo identificado en su máquina —los pasos 1 a 4 de acá arriba—,
> pero al resto se le sigue preguntando.
...
### A.6 · Un título mal declarado ya no se lleva puesto al resto (PLE-170) 🔴

**Qué estaba mal, y es lo que encontró Yael:** si un título no estaba ni en nuestro catálogo ni
en IGDB, se lo podía enlazar a cualquier juego de Steam. Y ese enlace quedaba escrito para
todos: cualquier persona que después capturara ese mismo ejecutable veía el título equivocado,
en silencio y sin cuestionario.

Pasó de verdad. Había quedado `supertux2.exe` enlazado a *Worm.is: The Game*,
`little runmo - the game.exe` a *Left 4 Dead 2* y `solitaire.exe` a *Stray*. **Ya están
corregidos en la base**; esta prueba es para confirmar que no se pueda volver a hacer.

#### A.6.1 · Declarar mal ya no ata el ejecutable

1. Abrir un título que el Recorder no reconozca y llegar al cuestionario.
2. **A propósito, declararlo mal**: elegir de la lista, o pegar el enlace de IGDB o de Steam,
   de un juego que **no** sea el que están corriendo.

**Esperado:** el Recorder acepta la declaración y **los deja grabar** — eso no cambió. Lo que sí
tiene que pasar es que ese enlace **no quede pegado al ejecutable**.

3. Para comprobarlo: cerrar el Recorder, **abrirlo con otra cuenta de QA** (otro
   `testhfrog+…`), en la misma máquina y con el mismo título abierto.

**Esperado:** a la segunda cuenta le vuelve a aparecer **el cuestionario**. No tiene que ver el
título equivocado que declaró la primera.

> 🔴 **Si la segunda cuenta ve el título equivocado ya resuelto, el bug sigue vivo.**

4. Ahora, desde esa segunda cuenta, declarar **lo mismo** que declaró la primera.

**Esperado:** a partir de acá sí queda enlazado — dos personas distintas diciendo lo mismo es lo
que lo confirma. Si prueban con una **tercera** cuenta, ya se lo reconoce solo.

> Declarar dos veces desde **la misma** cuenta no cuenta como confirmación. Si lo prueban y
> queda enlazado con una sola cuenta, repórtenlo.

#### A.6.2 · Se puede corregir un título equivocado

1. Con un título ya identificado en pantalla, mirar debajo del panel.

**Esperado:** un enlace gris que dice **"No es este título"**.

2. Tocarlo.

**Esperado:** se abre el cuestionario de nuevo, con el ejecutable correcto en el recuadro
**OBS ESTÁ CAPTURANDO**. Antes de esta versión no había ninguna forma de corregirlo desde el
Recorder.

---

### A.7 · Cambian dos umbrales de calidad de la sesión ⚠️

Estos dos no son arreglos de bugs: son cambios de criterio, y **van a hacer que algunas
sesiones que antes pasaban ahora no pasen**. Es lo esperado, no un bug.

| | Antes | Ahora |
|---|---|---|
| Tiempo continuo sin tocar teclado ni mouse | 10 min | **5 min** |
| Actividad mínima de teclado y mouse | prácticamente cualquiera | **20 eventos por minuto** |

1. Grabar 3 minutos **jugando normalmente** y detener.

**Esperado:** pasa la verificación sin observaciones.

2. Grabar 3 minutos **casi sin tocar nada** (dejar el título quieto, mover el mouse cada tanto).

**Esperado:** la sesión queda marcada como sin actividad suficiente. **Eso es correcto.**

3. Grabar dejando **más de 5 minutos seguidos** sin tocar teclado ni mouse.

**Esperado:** queda marcada como AFK. Antes hacía falta llegar a 10 minutos.

4. Correr el **Synch Checker** sobre las mismas sesiones.

**Esperado:** dice lo mismo que el Recorder. 🔴 **Si el Checker dice "SESIÓN LISTA PARA ENVIAR"
sobre una sesión que después se rechaza, repórtenlo enseguida**: significa que los dos quedaron
con umbrales distintos, que es un problema que ya tuvimos.
...
|---|---|---|
| 1 | Actualizar desde una versión anterior con el Updater | Abre en v0.9.10, con los acentos y los íconos bien |
...
- **Que una sesión con poca actividad de teclado y mouse quede observada.** Cambió el criterio en esta versión: ver A.7.
- **Que al declarar un título el ejecutable no quede enlazado hasta que otra persona declare lo mismo.** Es el arreglo de A.6, no una falla.
